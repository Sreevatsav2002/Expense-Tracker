...
def create_app():
    ...
    @app.route('/expense_statements')
    @login_required
    def expense_statements():
        user_expense_data = list(expense_collection.find({'user_id': current_user.id}))

        for expense in user_expense_data:
            expense_date_time = datetime.strftime(expense['timestamp'], "%Y-%m-%d %H:%M:%S")
            ex = expense_date_time.split(" ")
            expense['date'] = ex[0]
            expense['time'] = ex[1]

        return render_template('expense_statements.html', expense_data=user_expense_data)
